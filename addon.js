...
// if loaded through PimpMyStremio use the internal proxy
let proxy;
try {
...
// Mapping of categories to their identifiers
const categories = {
	'Animation': 'animation',
	'Auto': 'auto',
	'Business': 'business',
...
const manifest = {
	...
	catalogs: [{
		...
		extra: [
			{
				name: "genre",
				options: Object.keys(countries).concat(Object.keys(categories)),
				isRequired: true
			}
		]
	}, {
...
function m3uToMeta(data, category) {
	if (category === undefined) category = 'search';
...
function getData(category) {
	...
	return new Promise((resolve, reject) => {
		...
		if (categories[category]) {
			url = 'https://iptv-org.github.io/iptv/categories/' + categories[category] + '.m3u';
		} else if (countries[category]) {
			url = 'https://iptv-org.github.io/iptv/countries/' + countries[category] + '.m3u';
		} else {
		...
		request(url, function (error, response, body) {
			if (error) {
				...
				resolve(m3uToMeta(body, category));
			}
		});
	});
}
...
builder.defineStreamHandler(function (args, cb) {
	if (args.type === 'tv' && args.id.startsWith('iptvorg:')) {
		return new Promise((resolve, reject) => {
			const category = args.id.split(':', 2)[1].split('::')[0];
			getData(category).then(function (values) {
				for (let i = 0; i < values.length; i++) {
